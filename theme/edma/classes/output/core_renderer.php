...
class core_renderer extends \core_renderer {
   ...
   */
    ...
    public function get_theme_image_banner_shape_image3($maxwidth = null, $maxheight = 100) {
        global $CFG;
        if (!empty($this->page->theme->settings->banner_shape_image3)) {
            $url = $this->page->theme->setting_file_url('banner_shape_image3', 'banner_shape_image3');
            // Get a URL suitable for moodle_url.
            $relativebaseurl = preg_replace('|^https?://|i', '//', $CFG->wwwroot);
            $url = str_replace($relativebaseurl, '', $url);
            return new moodle_url($url);
            return parent::get_theme_image_banner_shape_image3($maxwidth, $maxheight);
        }
    }

    public function get_theme_image_footer_shape_image1($maxwidth = null, $maxheight = 100) {
        global $CFG;
        if (!empty($this->page->theme->settings->footer_shape_image1)) {
            $url = $this->page->theme->setting_file_url('footer_shape_image1', 'footer_shape_image1');
            // Get a URL suitable for moodle_url.
            $relativebaseurl = preg_replace('|^https?://|i', '//', $CFG->wwwroot);
            $url = str_replace($relativebaseurl, '', $url);
            return new moodle_url($url);
            return parent::get_theme_image_footer_shape_image1($maxwidth, $maxheight);
        }
    }

    public function get_theme_image_footer_shape_image2($maxwidth = null, $maxheight = 100) {
        global $CFG;
        if (!empty($this->page->theme->settings->footer_shape_image2)) {
            $url = $this->page->theme->setting_file_url('footer_shape_image2', 'footer_shape_image2');
            // Get a URL suitable for moodle_url.
            $relativebaseurl = preg_replace('|^https?://|i', '//', $CFG->wwwroot);
            $url = str_replace($relativebaseurl, '', $url);
            return new moodle_url($url);
            return parent::get_theme_image_footer_shape_image2($maxwidth, $maxheight);
        }
    }
    ...
    public function render_lang_menu() {
        ...
        if ($haslangmenu) {
            ...
            if (isset($context)) {
                return $this->render_from_template('theme_edma/lang_menu', $context);
            }
        }
    }
    ...
    public function if_home_pages() {
        ...
        $hide_banner    = get_config('theme_edma', 'hide_banner');
    ...
    public function if_hide_page_bottom_content() {
        ...
        $hide_page_bottom_content      = get_config('theme_edma', 'hide_page_bottom_content');
    ...
    public function banner_shape_image() {
        $url = new moodle_url('/theme/edma/pix/img/shape8.svg');
        return $url;
    }
     ...
     * @return string
     */
    public function edma_if_user_enrolled() {
     ...
     * @return string
     */
    public function edma_enroll_link() {
        $actual_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        $id = $_GET['id'];
        global $CFG;
     ...
     * if_guest
     */
    public function if_edma_guest() {
        $hide_guest_access_curriculum       = '';
        $hide_guest_access_curriculum       = theme_edma_get_setting('hide_guest_access_curriculum');
    ...
    public function get_pix_image_url_base() {
        ...
        return $CFG->wwwroot . "/theme/edma/pix";
    }
    ...
    public function get_footer_bottom() {
        $footer_copyright       = '';
        $footer_copyright       = theme_edma_get_setting('footer_copyright',  'format_html');
        ob_start();
        ?>
    ...
    public function get_footer_style() {
        $scsspre    = theme_edma_get_setting('scsspre');
        $scss       = theme_edma_get_setting('scss');
        ob_start();
        if($scsspre || $scss):
            ?>
                <style>
    ...
    public function search_box($id = false) {
        global $CFG;
        $search_placeholder = '';
        $search_placeholder = theme_edma_get_setting('search_placeholder', true);
        $wwwroot = $CFG->wwwroot;
        ...
        if ($withlinks) {
            ...
            foreach ($opts->navitems as $key => $value) {
                ...
                switch ($value->itemtype) {
                    ...
                    case 'link':                        
                        $edma_nav_items .= '<a class="dropdown-item" href="'. $value->url .'">'. $value->title .'</a>';
    ...
    */
    ...
    public function edma_is_siteadmin() {
        if (is_siteadmin()) {
     ...
     */
    public function edma_region_main_settings_menu() {
        global $OUTPUT;
