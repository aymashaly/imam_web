...
include($CFG->dirroot . '/theme/edma/inc/edma_globalsearch_navbar.php');
require_once($CFG->dirroot. '/theme/edma/inc/edma_page_handler.php');

// Add block button in editing mode.
$addblockbutton = $OUTPUT->addblockbutton();

$edmaPageHandler = new edmaPageHandler();
$pageheading = $edmaPageHandler->edmaGetPageTitle();
...
$edmaUserBodyClass = 'edma_body_class';
$extraclasses = array(
    'edma_no_hero',
    $user_status,
    $edmaUserBodyClass
...
if($isloggedin){
    $edmaProfileIconUsername = $USER->firstname . ' '. $USER->lastname;
}else{
    $edmaProfileIconUsername = '';
}
...
if($favicon) {
    ...
    $favicon = $CFG->wwwroot . '/theme/edma/pix/favicon.ico';
}

$hide_global_banner    = get_config('theme_edma', 'hide_global_banner');

if (get_config('theme_edma', 'logo_visibility') == 1) {
 ...
 * Main Logo
 */
$logo_image_width  = preg_replace("/[^0-9]/", "", get_config('theme_edma', 'logo_image_width'));
$logo_image_height = preg_replace("/[^0-9]/", "", get_config('theme_edma', 'logo_image_height'));
 ...
 * Mobile Logo
 */
$mobile_logo_width  = preg_replace("/[^0-9]/", "", get_config('theme_edma', 'mobile_logo_width'));
$mobile_logo_height = preg_replace("/[^0-9]/", "", get_config('theme_edma', 'mobile_logo_height'));
 ...
 * Footer Logo
 */
if (get_config('theme_edma', 'footer_logo_visibility') == 1) {
    $footer_logo_visibility = false;
} else {
    $footer_logo_visibility = true;
}
$footer_logo_width  = preg_replace("/[^0-9]/", "", get_config('theme_edma', 'footer_logo_width'));
$footer_logo_height = preg_replace("/[^0-9]/", "", get_config('theme_edma', 'footer_logo_height'));
...
$top_header             = get_config('theme_edma', 'top_header');
$top_header_right_content = get_config('theme_edma', 'top_header_right_content');
$top_header_content     = get_config('theme_edma', 'top_header_content');
$header_search          = get_config('theme_edma', 'header_search');
$header_btn_text        = get_config('theme_edma', 'header_btn_text');
$header_btn_url         = get_config('theme_edma', 'header_btn_url');
$header_left_btn_text   = get_config('theme_edma', 'header_left_btn_text');
$header_left_btn_url    = get_config('theme_edma', 'header_left_btn_url');
$offcanvas_content      = get_config('theme_edma', 'offcanvas_content');
$offcanvas_social_title = get_config('theme_edma', 'offcanvas_social_title');
$header_btn_icon        = get_config('theme_edma', 'header_btn_icon_edma_icon_class');

$social_target = get_config('theme_edma', 'social_target');
if($social_target == 1) {
...
$footer_copyright      = get_config('theme_edma', 'footer_copyright');
$hide_guest_access_curriculum    = get_config('theme_edma', 'hide_guest_access_curriculum');
...
$banner_shape_image3 = $OUTPUT->get_theme_image_banner_shape_image3(null, 100);
if($banner_shape_image3) {
    $banner_shape_image3 = $OUTPUT->get_theme_image_banner_shape_image3(null, 100);
}else{
    $banner_shape_image3 = false;
}

$footer_shape_image1 = $OUTPUT->get_theme_image_footer_shape_image1(null, 100);
if($footer_shape_image1) {
    $footer_shape_image1 = $OUTPUT->get_theme_image_footer_shape_image1(null, 100);
}else{
    $footer_shape_image1 = false;
}

$footer_shape_image2 = $OUTPUT->get_theme_image_footer_shape_image2(null, 100);
if($footer_shape_image2) {
    $footer_shape_image2 = $OUTPUT->get_theme_image_footer_shape_image2(null, 100);
}else{
    $footer_shape_image2 = false;
}


$back_to_top = get_config('theme_edma', 'back_to_top');
...
if(get_config('theme_edma', 'footer_col_1_body')){
    $footer_column_count++;
    $footer_column_1 = true;
}
if(get_config('theme_edma', 'footer_col_2_title') || get_config('theme_edma', 'footer_col_2_body')){
    $footer_column_count++;
    $footer_column_2 = true;
}
if(get_config('theme_edma', 'footer_col_3_title') || get_config('theme_edma', 'footer_col_3_body')){
    $footer_column_count++;
    $footer_column_3 = true;
}
if(get_config('theme_edma', 'footer_col_4_title') || get_config('theme_edma', 'footer_col_4_body')){
    $footer_column_count++;
    $footer_column_4 = true;
}
if(get_config('theme_edma', 'footer_col_5_title') || get_config('theme_edma', 'footer_col_5_body')){
    $footer_column_count++;
