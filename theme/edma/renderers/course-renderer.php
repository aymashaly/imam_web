 ...
 * Course renderer
 */
class theme_edma_core_course_renderer extends core_course_renderer {
    /**
    ...
    protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        $edma_category_link = new moodle_url('/course/index.php', array('categoryid' => $coursecat->id));

        $edma_cat = $coursecat->id;
        $edma_cat_summary_unclean = $chelper->get_category_formatted_description($coursecat);

        if ($edma_cat_summary_unclean !== null) {
            $edma_cat_summary = preg_replace("/<img[^>]+\>/i", " ", $edma_cat_summary_unclean);
        } else {
            $edma_cat_summary = '';
        }
        ...
        if ($coursecat->get_children_count() > 0) {
            $edma_items_count .= $coursecat->get_children_count() . ' ' . get_string('categories');
        } else {
            $edma_items_count .= count($coursecat->get_courses()) . ' ' . get_string('courses');
        }
        $edma_cat_updated = get_string('modified') . ' ' . userdate($coursecat->timemodified, '%d %B %Y', 0);
        ...
        if (isset($src)){
            if($src){
                ...
                $contentimages = '<img class="img-whp" src="'.$CFG->wwwroot.'/theme/edma/pix/category.jpg">';
            }
        } else {
            $contentimages = '<img class="img-whp" src="'.$CFG->wwwroot.'/theme/edma/pix/category.jpg">';
        ...
        $content = '';
        $content .= '
        <div class="col-lg-3 col-md-6 wow animate__animated animate__fadeInUp delay-0-2s">            
            <div class="courses_list_content">
                <div class="single-courses cat-card-'.$edma_cat.'">
                    <a href="'.$edma_category_link.'" class="courses-main-img">
                        '.$contentimages.'
                    </a>
                    ...
                    <div class="courses-content">
                        ...
                        <h4><a href="'. $edma_category_link .'">'.$categoryname.'</a></h4>
                        <ul class="admin">
                            <li>
                                <a href="'. $edma_category_link .'">
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course) {
        ...
        $edmaCourse = $edmaCourseHandler->edmaGetCourseDetails($courseid);
        ...
        $edma_context = context_course::instance($course->id);
        $numberofusers = count_enrolled_users($edma_context);
        ...
        if ($course->has_course_contacts()) {
            ...
            foreach ($course->get_course_contacts() as $coursecontact) {
                ...
                $edma_course_contacts .= '<span class="edma_course_meta_item mr10">'.$name.'</span>';
            }
        }
        ...
        $edmaCourseDescription = strip_tags($edmaCourseHandler->edmaGetCourseDescription($course->id, 99999999999999));
        $edmaCourseDescription = substr($edmaCourseDescription, 0, 98);
                ...
                <div class="single-courses">
                    ...
                    <div class="courses-content">
                        ...
                        <ul class="admin">
                            ...
                            <li>
                                <span>'.get_string('total_student', 'theme_edma').' </span>
                            </li>
                            <li>
                                <a href="'. $edmaCourse->url .'">
                        ...
                        <ul class="rating">
                            <li>
                                <i class="flaticon-clock"></i> '.get_string('last_updated', 'theme_edma').' '. $edmaCourse->edmaRender->updatedDate .'
                            </li>
                        </ul>';
                            ...
                            <div>
                                ...
                                <div class="courses-btn d-flex justify-content-between align-items-center">
                                    <a href="'. $edmaCourse->url .'" class="default-btn">
                                        '.get_config('theme_edma', 'enroll_btn') .'
                                    </a>
