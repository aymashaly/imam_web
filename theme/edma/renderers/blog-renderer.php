 ...
 * Blog renderer
 */
class theme_edma_core_blog_renderer  extends core_blog_renderer  {
    ...
    public function render_blog_entry(blog_entry $entry) {
        ...
        if ( strpos($actual_link, '/index.php?entryid') == false ):
            // Header.
            $mainclass = 'col-xl-4 col-md-6 wow animate__animated animate__fadeInUp delay-0-2s ';
            ...
            $o = $this->output->container_start($mainclass, 'b' . $entry->id);
                $o .= $this->output->container_start('single-blog', '' . $entry->id);

                    $o .= $this->output->container_start('blog-img');
                        // Attachments.
                        ...
                        $o .= $this->output->container_start('date');
                    ...
                    $o .= $this->output->container_start('blog-content');
                        ...
                        $o .="<ul class='d-flex justify-content-between'>";
                            $o .="<li class='admin'>";
                                // Post by.
                                $by = new stdClass();
            ...
            $o = $this->output->container_start($mainclass, 'b' . $entry->id);
                ...
                if (!empty($entry->renderable->comment)) {
                    ...
                    $edma_comments = new comment($cmt);
                    $edma_comments->set_view_permission(true);
                    $edma_comments->set_fullwidth();

                    $o .= $edma_comments->output(true);
                }
            $o .= $this->output->container_end();
