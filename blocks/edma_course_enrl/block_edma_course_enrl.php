...
class block_edma_course_enrl extends block_base {
    public function init() {
        $this->title = get_string('pluginname', 'block_edma_course_enrl');
    }
    ...
    public function get_content() {
        ...
        $edmaCurrentUserId = $USER->id;

        $context = context_course::instance($COURSE->id, MUST_EXIST);

        $edmaEnroled = get_string('course_enrolled', 'theme_edma');
        $edmaEnroledText = get_string('course_enrolled_text', 'theme_edma');

        if( function_exists('isguestuser')
          && !isguestuser()
          && isloggedin()
          && is_enrolled($context, $USER, '', true)
          && isset($edmaCourse->teachers[$edmaCurrentUserId])
          && ($edmaCurrentUserId == $edmaCourse->teachers[$edmaCurrentUserId]->userId)
          ){
            $edmaEnroled = get_string('course_enrolled_teacher', 'theme_edma');
            $edmaEnroledText = get_string('course_enrolled_teacher_text', 'theme_edma');
        }
        ...
        <div class="course-details-sidebar">
            ...
            <div class="sidebar-futcher">';
                if($this->content->price){
                    ...
                    }elseif($edmaCourse->course_price) {
                        $text .='
                        <div class="sidebar-title d-flex justify-content-between">
                            <h2> ' . get_config('theme_edma', 'site_currency') .'' . $edmaCourse->course_price .'</h2> 
                ...
                if(!$edmaCourse->course_is_enrolled):
                    $text .='
                    <div class="cart-wish d-flex justify-content-between">
                        <a href="'. $edmaCourse->enrolmentLink .'" class="default-btn">
                        '. get_string('course_enrolment', 'theme_edma') .'
                        </a>
                    </div>';
                endif;
                $text .='
